class Temp:
    ...
    def modify_qp(self,qp,Vol,Dose,Weight,Opti_all,Surf_margin,status):
        ...
        self.qp[21] = ''.join(['        status=',str(status),'\n'])      
        self.qp[-2] = ''.join(['    !END\n'])
        
        return self.qp[:-1]
    
    def modify_po(self,po,Dose,alpha,status): 
        
        self.po = po
        self.po[18] = ''.join(['        isoconstraint=',str(Dose),'\n'])
        self.po[10] = ''.join(['        alpha=',str(alpha),'\n'])
        self.po[21] = ''.join(['        status=',str(status),'\n'])   
        self.po[-2] = ''.join(['    !END\n'])
        
        return self.po[:-1]    
    
    
    def modify_se(self,se,Dose,Weight,Shrink_margin,Opti_all,Powe_Law,status):
        ...
        self.se[21] = ''.join(['        status=',str(status),'\n'])  
        self.se[-2] = ''.join(['    !END\n'])
        
        return self.se[:-1]


    def modify_pa(self,pa,Ref_dose,Volume,Weight,Powe_Law,Opti_all,Shrink_margin,status):
        ...
        self.pa[21] = ''.join(['        status=',str(status),'\n'])           
        self.pa[-2] = ''.join(['    !END\n'])
        
        return self.pa[:-1]

    def modify_mxd(self,mxd,Dose,Weight,Opti_all,Shrink_margin,status):
        ...
        self.mxd[-2] = ''.join(['    !END\n'])
        self.mxd[21] = ''.join(['        status=',str(status),'\n']) 

        return self.mxd[:-1]

    def modify_qod(self,qod,Dose,RMS,Shrink_margin,status):
        ...
        self.qod[-2] = ''.join(['    !END\n']) 
        self.qod[21] = ''.join(['        status=',str(status),'\n']) 
        ...
        for i,j in enumerate(OBJ[1]):
            ...
            if j[0][0] == 'D':
                ...
                if j[0] == 'Dmean':
                    ...
                    se = self.modify_se(path_new['se'], j[1], weight_OARs, 0, 0, 1,1)        
                    self.cost_fun.extend(se)
                    
                elif j[0] == 'Dmax':
                    
                    mxd =self.modify_mxd(path_new['mxd'], j[1], weight_OARs, 0, 0, 1)
                    self.cost_fun.extend(mxd)
                
                else:
                    
                    se = self.modify_se(path_new['se'], j[1]*0.75, weight_OARs, 0, 0, 16,1)
                    self.cost_fun.extend(se)
                ...
                if flag <= 15.0:               
                    se = self.modify_se(path_new['se'], s*0.75, weight_OARs, 3, 0, k_se,1)
                    self.cost_fun.extend(se)
                else:
                    pa = self.modify_pa(path_new['pa'], s, flag, weight_OARs, k_pa, 0, 0,1)
...
#                prep_v = float((item[1][0][1]+3)/100)
    ...
    def get_initial_prostate(self,strt_ind_list,path_beam,dose_frac):
...
#                prep_v = float((item[1][0][1]+3)/100)
                prep_d = float(item[1][0][0][1:])
                # tar_pen = self.modify_po(path_new['po'],prep_d,0.6)
                tar_pen = self.modify_qp(path_new['qp'],0.98,prep_d,1.00,0,0,1)
                qod = self.modify_qod(path_new['qod'],prep_d+2,RMS,0,1)
                target = target + part2 + tar_pen + qod
                target.append('!END\n')
...
#                prep_v = float((item[1][0][1]+3)/100)
                prep_d = float(item[1][0][0][1:])
                # po = self.modify_po(path_new['po'],prep_d,0.6)
                po = self.modify_qp(path_new['qp'],0.98,prep_d,1.00,0,0,1)
                ## set two quadratic overdose to external targets
                qod1 = self.modify_qod(path_new['qod'],pres,RMS,0,1)
    #            QOD2 = temp_tool.modify_QOD(QOD_path,prep_d-4,RMS1,grid)
                qod2 = self.modify_qod(path_new['qod'],prep_d*1.1,RMS,0,1)
        ...
        for item in strt_ind_list:
            ...
            if item[0] not in tar_nam:
                ...
                if item[-1] == 5: ##　stem and cord
                    ...
                    part2[1] = '    name=' + item[0] +'\n'
                    ## global maximum dose
                    mxd1 = self.modify_mxd(path_new['mxd'], round(pres*1.06,2), weight_OARs, 1, 0,1)
                    ## the outer target dose
                    QOD1 = self.modify_qod(path_new['qod'],max_dose,RMS,0,1)
                    QOD2 = self.modify_qod(path_new['qod'],max_dose*0.8,RMS,0,1)
                    QOD3 = self.modify_qod(path_new['qod'],max_dose*0.6,RMS,0,1)
                    OARs = OARs + part2 + mxd1 + QOD1 + QOD2 + QOD3
                    OARs.append('!END\n')
...
#        stat = input('Do you want to change the name? if Yes enter 1 & No enter 0\n')
        ...
        if selection == '1':
            ...
            template = self.get_initial_prostate(self.strt_ind_list,path_beam,dose_frac)
