# -*- coding: utf-8 -*-
"""
Created on Mon Jan  7 10:05:15 2019
...
class Tool:
    ...
    '''
    def __init__(self,path,ind_loc_strt,iter_path,sess):
        from AC import Worker
    ...
    def csv_read_to_dvh(self):
        ...
        dvh = [[item[0].replace(' ',''),item[1].replace(' ',''), 
        item[2].replace(' ','')]for item in dvh_ ]
#        dvh = []
#        for i in dvh_:
#            kk = i[0].split(' ')
...
#        dose_frac = [l for l in prep if l[0] == 'prep' or l[0] == 'frac' ]
    ...
    def score_fun(self,pres_strt_ind,dvh_indices,selection):
        
        '''
          This function mainly used for scoring the plan
          Selection = 1 -> Prostate
          Selection = 2 -> HeadNeck
        '''
        print('pres_strt_ind:',pres_strt_ind)
        print('DVH Statistics:',dvh_indices.keys())
        import numpy as np
        SCORE = 0
        if selection == '1':

            score,dvh_indices_score = {key: [] for key in pres_strt_ind},{key: [] for key in pres_strt_ind}
            
            # Define the known points
            for key, value in pres_strt_ind.items():
            
                if 'PTV' not in key:
                    item = value
            
                    for pp in item:
                        x = [0.6*pp[1],pp[1],1.5*pp[1]]
                        y = [15,5,-20]
                        # Calculate the coefficients. This line answers the initial question. 
                        coefficients1 = np.polyfit(x[0:2], y[0:2], 1)
                        coefficients2 = np.polyfit(x[1:3], y[1:3], 1)
                        # Print the findings
                        print (key,pp)
                        print ('f1:y = {}x + {}'.format(coefficients1[0],coefficients1[1]))
                        print ('f2:y = {}x + {}'.format(coefficients2[0],coefficients2[1]))
                        score[key].append([pp,(coefficients1,coefficients2,x)])
                else:
                    item = value
                
                    for pp in item:
                        x = [pp[1]-5,pp[1],pp[1]+5]
                        y = [-20,15,25]
                        # Calculate the coefficients. This line answers the initial question. 
                        coefficients1 = np.polyfit(x[0:2], y[0:2], 1)
                        coefficients2 = np.polyfit(x[1:3], y[1:3], 1)
            ...
            total = sum([len(value) for key,value in score.items()]) 
            #    print("Number of the DVH indices:".format(total))
#            print(dvh_indices_score)
            ## DVH_indices to calculate the correspoding score
            for key,value in dvh_indices.items():
                if key in score.keys():
                    for item1,item2 in zip(dvh_indices[key],score[key]):
                
                        if item1[1] < item2[1][-1][0]:
                            dvh_indices_score[key].append((item1[0],item1[1],
                                item2[1][2][0] * item2[1][0][0] + item2[1][0][1]))
                
                        elif item1[1] <item2[1][-1][1]:
                            dvh_indices_score[key].append((item1[0],item1[1],
                                item1[1] * item2[1][0][0] + item2[1][0][1]))
                
                        elif item1[1] < item2[1][-1][-1]:
                            dvh_indices_score[key].append((item1[0],item1[1],
                                item1[1] * item2[1][1][0] + item2[1][1][1]))
                
                        else:
                            dvh_indices_score[key].append((item1[0],item1[1],
                                item2[1][2][-1] * item2[1][1][0] + item2[1][1][1]))
                
            print('dvh_indices_scire:',dvh_indices_score)
            
            x_axis = np.linspace(0,100,100)
            x = np.tile(x_axis,(total,1))
            y_axis = np.linspace(0,100,100)
            y = np.tile(y_axis,(total,1))
            kk = score["Bladder"][0]
            # print(x.shape)
            j = 0
            points,points_dvh = [],[]
            for key in score.keys():
                ...
                for kk,jj in zip(score[key],dvh_indices_score[key]): #kk = score[key][0] 
                
                    for i in range(len(x[j,:])):
                
                        if x[j,i] < kk[1][2][0]: # X<6
                            y[j,i] = kk[1][2][0] * kk[1][0][0] + kk[1][0][1]
                
                        elif x[j,i] < kk[1][2][1]: # 6<=X<10
                            y[j,i] = x[j,i] * kk[1][0][0] + kk[1][0][1]
                
                        elif x[j,i] < kk[1][2][2]: # 10<=X<15
                            y[j,i] = x[j,i] * kk[1][1][0] + kk[1][1][1]
                
                        else:   # X >= 15
                            y[j,i] = kk[1][2][-1] * kk[1][1][0] + kk[1][1][1]
                    
                    j += 1
                    points.append([(key,kk[0]),(kk[1][2][0],kk[1][2][0] * kk[1][0][0] + kk[1][0][1]),
                        (kk[1][2][1],kk[1][2][1] * kk[1][0][0] + kk[1][0][1]),
                        (kk[1][2][-1],kk[1][2][-1] * kk[1][1][0] + kk[1][1][1])])
                    points_dvh.append((jj[1],jj[2]))
                #    print(points)

            # fig = plt.figure(figsize = (20,20))
            for i in range(total):
#                    print('points:',points_dvh[i][1])
                SCORE += points_dvh[i][1]
            #     plt.subplot(3,4,(1+i))
            #     plt.plot(x[i,:],y[i,:])
            #     plt.plot(points[i][1][0],points[i][1][1],'go')
            #     plt.plot(points[i][2][0],points[i][2][1],'go')
            #     plt.plot(points[i][3][0],points[i][3][1],'go')
            #     plt.plot(points_dvh[i][0],points_dvh[i][1],'r*')
            #     plt.text(points_dvh[i][0],round(points_dvh[i][1],2),
            #      (points_dvh[i][0],round(points_dvh[i][1],2)),ha='center', va='bottom', fontsize=10)
            #     plt.grid()
            #     plt.xlabel(points[i][0], fontsize = 12, color = 'b')
            #     plt.ylabel("Score")
            # plt.show()
    ...
    def get_state(self): ## new written
        import numpy as np
        ...
        W = self.inf['wgt']
        R = self.inf['rlp']
        diff_ratio = self.diff_result

        size_inf = sum(map(lambda x: len(x), IE.values()))
        state_inf = np.zeros(4*size_inf)
        for key in IC.keys():
            for i,(ie, isc, wgt, rlp) in enumerate(zip(IE[key], IC[key], W[key], R[key])):
                state_inf[4*i] = ie[1]
                state_inf[4*i + 1] = isc[1]
                state_inf[4*i + 2] = wgt[1]
                state_inf[4*i + 3] = rlp[1]

        size_dvh = sum(map(lambda x: len(x), diff_ratio.values()))
        state_dvh = np.zeros(size_dvh)
        for key in diff_ratio.keys():
            for i,ratio in enumerate(diff_ratio[key]):
                state_dvh[i] = ratio[1]

        return state_inf, state_dvh

    def opt_agent(self, action, diff_result, tar_res_nam):
        import math
    ...
    #    count = 0
        ...
        size_action = sum(map(lambda x: len(x), IE.values()))
        action = np.zeros(size_action)

        count = 0
        for key, values in IC.items():
            for j in range(len(values)):
                action[count] = IC[key][j][1]
                count += 1
        ...
        for key, values in IC.items():
            for j in range(len(values)):
                action[count] = IC[key][j][1] - action[count]
                count += 1
...
#            flag = flag
    ...
    def exe1(self,tras,selection):
        ...
        if flag == '0':
            ...
            self.cur_score = self.score_fun(pres_strt_ind,dvh_indices,selection) ## to evaluate the plans

            print('Score:',self.cur_score)
    ...
    def exe2(self,tar_res_nam,selection,level_OARs,tras,iteration):
        '''
        ...
        strt_fun,strt_index,line,diff_result = self.exe1(tras,selection)
        ...
        ####================opt using AC=========================##
        self.state_inf, self.state_dvh = self.get_state()

        if iteration != 0:
            # update network at every 10 iterations
            print(iteration, self.cur_score, self.old_score)
            update = (iteration % 1 == 0)
            self.agent.update(self.old_state_inf, self.old_state_dvh, self.state_inf, self.state_dvh, self.action, self.cur_score-self.old_score, update=update)
        
        if np.random.rand() > 10.0: ## if random > 0.5 # /(iteration+1)

            print('Optimizing using RL\n')
            self.action = self.agent.predict(self.state_inf, self.state_dvh)
            inf, flag = self.opt_agent(self.action, diff_result, tar_res_nam)

        else:                                    ## if random < 0.5
            print('Optimizing using rule based\n')
            if selection == '1':
                ## 用来优化前列腺cases
    ...
    #            level_OARs
        ...
        self.old_state_inf, self.old_state_dvh = self.state_inf, self.state_dvh 
    ...
    def exe3(self,tar_res_nam,selection,level_OARs,tras,iteration):
        '''
           this function mainly execute the struct_fun's transformation
        '''
        struct_fun_updated,mark3,information,diff_result,line = self.exe2(tar_res_nam,selection,level_OARs,tras,iteration)
        ...
        return mark3,information,diff_result,self.cur_score
