    ...
    def hyp_solution_NPC_V1(self,grid,fractions,prescription_dose,delivery_type):
        ...
        for item in OARs_nam:
            ...
            if item == 'Brain Stem':
                ...
                # select CF: maximum                
                cf = self.modify_mxd(Dose=max_dose,Weight=0.01,Opti_all=1,Shrink_margin=0)
                ...
                if '%' in self.protocol_dict[item][0][0]:
                    ...
                    if percent < 10: 
                        ...
                        cf = self.modify_se(Dose= eud_dose,Weight=0.01,Shrink_margin=0,Opti_all=0,Powe_Law=12)                       
                ...
                eud_dose = 0.5*tar[0][-1]
                cf1 = self.modify_se(Dose= eud_dose,Weight=0.01,Shrink_margin=0.25,Opti_all=0,Powe_Law=12) 
                
            
                # select CF2: serial (constrain mean dose)
                eud_dose = float(self.protocol_dict[item][0][1].split('Gy')[0])
                cf = self.modify_se(Dose= eud_dose,Weight=0.01,Shrink_margin=0,Opti_all=1,Powe_Law=1) 
