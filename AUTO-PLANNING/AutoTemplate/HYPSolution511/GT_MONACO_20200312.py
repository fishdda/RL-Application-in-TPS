...
#        xml_path_output = 'C:/Users/xhuae08006/Desktop/XHTOMO_AUTO/Modifier/test_dosenormsettings.xml'
        ...
        if Opti_all == 1:
            ...
            for i,item in enumerate(self.element['# se\n']):
                ...
                if item.split('=')[0] == '        isoconstraint':
                    ...
                    for j,jtem in enumerate(self.tar_nam):
    ...
    def modify_qod_551(self,Dose,RMS,Shrink_margin,Opti_all):
    ...
    def hyp_solution_NPC_V2(self,grid,fractions,prescription_dose,delivery_type):
        ...
        for i,item in enumerate(tar):
            ...
            if i != len(tar)-1:  ## inner target 
                ...
                # second quadratic overdose to constarin 110% percent of external target dose region
                qod2 = self.modify_qod_551(Dose = int(item[2]*1.1),RMS = 0.5,Shrink_margin = grid*2, Opti_all= 0)
                target = target + qod2[:-1]
                
                # third quadratic overdose to constrain 102% percent of external target dose region
                qod3 = self.modify_qod_551(Dose = int(item[2]*1.02),RMS = 0.75,Shrink_margin = grid*3, Opti_all = 0)
                target = target + qod3[:-1]
        ...
        for item in OARs_nam:
            ...
            if item == 'Brain Stem':
                ...
                # select CF2: serial (constrain mean dose)
                eud_dose2 = float(self.protocol_dict[item][0][1].split('Gy')[0])
                cf2 = self.modify_se_551(Dose= eud_dose2,Weight=0.01,Shrink_margin=0,Opti_all=1,Powe_Law=1) 
                ...
                # select CF2: serial (constrain mean dose, eud = pro_dose+2Gy)
                eud_dose = float(self.protocol_dict[item][0][1].split('Gy')[0])
                cf2 = self.modify_se_551(Dose= int(eud_dose),Weight=0.01,Shrink_margin=0,Opti_all=0,Powe_Law=1) 
                ...
                # select CF2: parallel (constrain mean dose, eud = pro_dose+2Gy)
                max_dose = float(self.protocol_dict[item][0][1].split('Gy')[0])
                cf2 = self.modify_pa_551(Ref_dose= int(max_dose),Volume = 50, Weight=0.01,Powe_Law=4,Opti_all=1,Shrink_margin=0) 
                ...
                # select CF: Maximum (Constrain D5%)
                max_dose = float(self.protocol_dict[item][0][1].split('Gy')[0])
                cf = self.modify_mxd_551(Dose= int(max_dose)+5,Weight=0.01,Opti_all=1,Shrink_margin=0)
                ...
                # select CF1: Quadratic Overdose(Constrain D2cc/Max Dose)
                max_dose = tar[0][-1]
                cf1 = self.modify_qod_551(Dose= int(max_dose),RMS = 0.25,Shrink_margin = 0, Opti_all = 0) 
                # cf1 = self.modify_mxd(Dose=max_dose,Weight=0.01,Opti_all=1,Shrink_margin=0)
                # cf1 = self.modify_se(Dose= max_dose*0.75,Weight=0.01,Shrink_margin=0.25,Opti_all=0,Powe_Law=12) 
                OARs + part2 + cf1[:-1]
            ...
            # assistance structure like SPPRV,BSPRV,R6,R7
                ...
                ## global maximum dose
                mxd1 = self.modify_mxd_551(Dose= int(tar[0][-1]*1.1), Weight=0.01, Opti_all=1, Shrink_margin=0)
                OARs = OARs + part2 + mxd1[:-1]
                
                ## the outer target dose
                QOD1 = self.modify_qod_551(Dose = int(tar[-1][-1]), RMS = 0.5, Shrink_margin = 0, Opti_all=0)
                OARs = OARs + QOD1[:-1]
                
                QOD2 = self.modify_qod_551(Dose = int(tar[-1][-1])-5, RMS = 0.75, Shrink_margin = grid,Opti_all = 0)
                OARs = OARs + QOD2[:-1]

                QOD3 = self.modify_qod_551(Dose = int(tar[-1][-1])-10, RMS = 1, Shrink_margin = grid*2,Opti_all = 0)
                OARs = OARs + QOD3[:-1]

                QOD4 = self.modify_qod_551(Dose = int(tar[-1][-1])-15, RMS = 1.25, Shrink_margin = grid*3, Opti_all= 0)
                OARs = OARs + QOD4[:-1]

                QOD5 = self.modify_qod_551(Dose = int(tar[-1][-1])-20, RMS = 2.0, Shrink_margin = grid*4, Opti_all = 0)
                OARs = OARs + QOD5[:-1]                
